...
print.MCA <- function(obj, ...) {
  ...
  NAME <- c(
    "", "$cells_standard", "$cells_principal", "$genes_standard",
    "$genes_principal", "$Cell2Cell_Distance", "$Axis_Gene_Correlation",
    "$plot", "$eigen_value", "$Wilcoxon", "$Shiny"
  )
  DESCRIPTION <- c(
    "", "Cells raw coordinate", "Cells normalised coordinate",
  ...
  tibble(NAME, DESCRIPTION) %>% print.data.frame(
    row.names = FALSE,
    right = FALSE
  )
}
...
print.Cluster <- function(obj, ...) {
  ...
  NAME <- c(
    "", "$nClusters", "$labels", "$closest_cluster",
    "$coord_centroids", "$plot1", "$plot2"
  )
  DESCRIPTION <- c(
    "", "Number of Cluster", "A vector indicating the cluster for each samples",
