...
print.MCA <-function(obj, ...)
{
    ...
    NAME <- c("", "$cells_standard", "$cells_principal", "$genes_standard",
        "$genes_principal", "$Cell2Cell_Distance", "$Axis_Gene_Correlation",
        "$plot", "$eigen_value", "$Wilcoxon", "$Shiny")
    DESCRIPTION <- c("", "Cells raw coordinate", "Cells normalised coordinate",
        "Genes raw coordinate", "Genes normalised coordinate",
        "Distance Between Individuals in the Euclidean Space",
        "Pearson Correlation between cells coordinate and Gene Expression",
        "plot of Cell space", "eigenvalue", "Wilcoxon Test p-value for the number of axis to keep",
        "Interactive Visualisation of the MCA Results")
    tibble(NAME, DESCRIPTION) %>% print.data.frame(row.names = FALSE,
        right = FALSE)
}
...
print.Cluster <- function(obj, ...)
{
    ...
    NAME <- c("", "$nClusters", "$labels", "$closest_cluster",
        "$coord_centroids", "$plot1", "$plot2")
    DESCRIPTION <- c("", "Number of Cluster", "A vector indicating the cluster for each samples",
        "Table indicating for each genes the closest cluster centroids",
        "Coordinates of the Cluster Centroids", "Cluster plot in Cell Space",
...
print.GSEA <- function(obj, ...)
{
    ...
    DESCRIPTION <- c("", "Table with Gene Ranking for each cluster",
