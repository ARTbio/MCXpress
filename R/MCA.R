## MCA Library library(Rtsne) library(fields) library(igraph)
library(reshape2)
select <- dplyr::select
...
Dimension_reduction_MCA <- function(X, Dim = 5) {
  ...
  X$Dim_Red$Cells_Standard <- MCA$li %>% set_colnames(Component) %>% data.frame

  X$Dim_Red$Cells_Principal <- MCA$l1 %>% set_colnames(Component) %>% data.frame

  X$Dim_Red$Genes_Standard <- MCA$co %>% set_colnames(Component) %>% data.frame

  X$Dim_Red$Genes_Principal <- MCA$c1 %>% set_colnames(Component) %>% data.frame
  ...
  X$Dim_Red$Cell2Cell_Distance %<>% set_rownames(X$Dim_Red$Cells_Principal %>%
                                                   rownames)
  X$Dim_Red$Cell2Gene_Distance <- rdist(X$Dim_Red$Cells_Principal, X$Dim_Red$Genes_Standard) %>%
    set_colnames(X$Dim_Red$Genes_Standard %>% rownames) %>% set_rownames(X$Dim_Red$Cells_Principal %>%
                                                                           rownames)
  # End Distance Calculation
  X$Dim_Red$Eigen_Value <- Eig %>% set_names(Component)
  EigCum <- ((MCA$eig * 100)/(MCA$eig %>% sum)) %>% as.data.frame() %>% mutate(Axis = (paste0("Axis",
                                                                                              c(1:length(MCA$eig))))) %>% mutate(Cumul = ((MCA$eig * 100)/(MCA$eig %>%
                                                                                                                                                             sum)) %>% cumsum)
  ...
  for (i in 1:((X$Dim_Red$Cells_Standard %>% ncol) - 1)) {
    Wil <- wilcox.test(X$Dim_Red$Cells_Standard[, 1:i] %>% rdist %>% as.vector,
                       X$Dim_Red$Cells_Standard[, 1:(i + 1)] %>% rdist %>% as.vector, alternative = "less",
                       paired = TRUE)
  ...
  X$Dim_Red$Axis_Gene_Cor <- cor(X$Disjunctive_Matrix, X$Dim_Red$Cells_Principal) %>%
    data.frame %>% rownames_to_column(var = "Genes")
  # End Calculate Correlation Axis and Genes

  X$Dim_Red$Graph <- X$Dim_Red$Cells_Principal %>% ggplot(aes(x = Axis1, y = Axis2,
                                                              text = row.names.data.frame(X$Dim_Red$Cells_Principal))) + geom_point() +
    theme_light()
...
Dimension_reduction_MCA_FAST <- function(X, Dim = 5) {
  ...
  Component <- paste0("Axis", 1:Dim)
  X$Dim_Red$Cells_Principal <- (sqrt(nrow(MCA)) * V)[, 1:Dim] %>% set_colnames(Component) %>%
    set_rownames(MCA %>% rownames) %>% data.frame  ################l1
  X$Dim_Red$Cells_Standard <- (sqrt(nrow(MCA)) * (V %*% sqrt(D)))[, 1:Dim] %>%
    set_colnames(Component) %>% set_rownames(MCA %>% rownames) %>% data.frame  ##### li
  X$Dim_Red$Genes_Standard <- ((t(Z) %*% V) * Dc)[, 1:Dim] %>% set_colnames(Component) %>%
    set_rownames(MCA %>% colnames) %>% data.frame
  X$Dim_Red$Genes_Principal <- sweep(X$Dim_Red$Genes_Standard, 2, sqrt(Eig)[2:(Dim +
                                                                                 1)], "/") %>% set_rownames(MCA %>% colnames) %>% data.frame
  # Distance Calculation
  cat("Calculating Cell to Cell Distance...\n")
  X$Dim_Red$Cell2Cell_Distance <- X$Dim_Red$Cells_Standard[, 1:Dim] %>% rdist
  X$Dim_Red$Cell2Cell_Distance %<>% set_colnames(X$Dim_Red$Cells_Principal %>%
  ...
  EigCum <- ((Eig * 100)/(Eig %>% sum)) %>% as.data.frame() %>% mutate(Axis = (paste0("Axis",
                                                                                      c(1:length(Eig))))) %>% mutate(Cumul = ((Eig * 100)/(Eig %>% sum)) %>% cumsum)
  colnames(EigCum)[1] <- "EigenValue"
  EigCum <- EigCum[c(2, 1, 3)]
  EigCum <- EigCum[1:(X$Dim_Red$Cells_Standard %>% nrow), ] %>% na.omit
  X$Dim_Red$Cumul <- EigCum %>% gather(EigenValue, Cumul, key = "Type", value = "Value")
  ...
  for (i in 1:((X$Dim_Red$Cells_Standard %>% ncol) - 1)) {
    ...
    dis2 <- X$Dim_Red$Cells_Standard[, 1:(i + 1)] %>% rdist %>% as.matrix %>%
      as.vector
    Wilcox[i] <- wilcox.test(dis1, dis2, alternative = "less", paired = TRUE) %>%
      extract2("p.value")
    names(Wilcox)[i] <- paste0("Axis", i, "-Axis", i + 1)
  }
  X$Dim_Red$Wilcoxon <- tibble(Wilcox %>% names, Wilcox) %>% set_colnames(c("Axis", "p_value"))
  # End Calculate Wilcoxon for Cell2CellDistance
  ...
  X$Dim_Red$Plot <- X$Dim_Red$Cells_Principal %>% ggplot(aes(x = Axis1, y = Axis2)) +
