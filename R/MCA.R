...
MCA <- function(X, Dim = (X$ExpressionMatrix %>% ncol) - 1)
{
    ...
    X$MCA$Cell2Cell_Distance <- X$MCA$cells_standard[, 1:Dim] %>%
        rdist %>% set_colnames(X$MCA$cells_principal %>% rownames) %>%
    ...
    X$MCA$explained_eigen_variance <- tibble(factor(Axes, levels = Axes),
        Percentage_Variance, Percentage_Variance_Cum) %>% set_colnames(c("Axis",
    ...
    X$MCA$Axis_Gene_Cor <- cor(X$Disjunctive_Matrix, X$MCA$cells_principal,
        method = "spearman") %>% data.frame() %>% rownames_to_column(var = "Genes") %>%
        as_tibble()
    X$MCA$Axis_Gene_Cor[, -1] <- X$MCA$Axis_Gene_Cor[, -1] %>%
        map_df(.f = round, digits = 3)
    # End Calculate Correlation Axis and Genes . . . . . . . . .
    # . . . . . . . . . .  . . . . . . . . . . . . . . . . . . ..
    # Cell Principal Space ####

    X$MCA$plot <- X$MCA$cells_principal %>% ggplot2::ggplot(ggplot2::aes(x = Axis1,
        y = Axis2)) + ggplot2::geom_point() + ggplot2::theme_light()
