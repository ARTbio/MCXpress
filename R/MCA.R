...
MCA <- function(X, Dim = (X$ExpressionMatrix %>% dim %>% min) - 1) {
    ...
    X$MCA$cells_principal <- (sqrt(nrow(X$disjunctive_matrix)) * V)[, 1:Dim] %>%
        set_colnames(Component) %>% set_rownames(X$disjunctive_matrix %>% rownames) %>%
        data.frame
    ...
    X$MCA$genes_standard <- ((t(Z) %*% V) * Dc)[, 1:Dim] %>% set_colnames(Component) %>%
        set_rownames(X$disjunctive_matrix %>% colnames) %>% data.frame
    setTxtProgressBar(pb, 0.9)
    X$MCA$genes_principal <- sweep(X$MCA$genes_standard, 2, sqrt(Eig)[1:(Dim)], "/") %>%
        set_colnames(Component) %>% set_rownames(X$disjunctive_matrix %>% colnames) %>%
        data.frame
    ...
    X$MCA$Axis_Gene_Cor[, -1] <- X$MCA$Axis_Gene_Cor[, -1] %>% map_df(.f = round,
        digits = 3)
