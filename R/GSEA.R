Functional_Analysis_GSEA <-
  function(X,
    ...
    axis_rank<-df %>%
      select(-Genes) %>%
      purrr::map(abs) %>%
      purrr::map(multiply_by,-1) %>%
      purrr::map(set_names, nm= df$Genes) %>%
      purrr::map(rank, ties.method= "first") %>%
      purrr::map(sort)

    axis_gsea<-axis_rank %>% purrr::map(.f = function(x){fgsea(
      pathways = GMTfile,
      stats = x ,
      nperm=nperm,
      maxSize=maxSize,
      ...
      nproc=nproc,
      BPPARAM = SerialParam(),
      gseaParam = 1)}) %>%  purrr::map(as_tibble)

    df2<- X$cluster$Gene_Cluster_Distance %>%
      separate(col= Genes, into = c("Genes","bin"),sep = "-bin", convert = TRUE) %>%
      filter(bin==1) %>%  select(-bin)
    ...
    cluster_gsea<-cluster_rank %>% purrr::map(.f = function(x){fgsea(
      pathways = GMTfile,
      stats = x ,
      nperm=nperm,
      maxSize=maxSize,
