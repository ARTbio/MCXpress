...
Df_To_Mat <- function(x) {
  Dframe <-
    x[, -1] %>%
    as.matrix() %>%
    set_rownames(x[, 1] %>% as.matrix() %>% as.vector())
  return(Dframe)
}
...
Mat_To_Df <- function(x, namecol = "Sample") {
  Mat <-
    x %>%
    as.data.frame() %>%
    tibble::rownames_to_column(var = namecol) %>%
    as_tibble()
  return(Mat)
}
...
Heatmap_Cluster_SC <- function(x, n = 5, plotly = F) {
  ...
  Lab <-
    ((TableCell[-1] + TableCell[-(TableCell %>% length())]) / 2) %>%
    add(0.5) %>%
    round(digits = 0)
...
Heatmap_Cluster <- function(X, n = 5, plotly = T) {
  ...
  HeatDF <-
    ...
    t() %>%
    Mat_To_Df("Genes") %>%
    gather("Cells", "Value", -Genes) %>%
    inner_join(X$cluster$labels %>% dplyr::rename(Cells = Sample))
  HeatMat <-
    HeatDF %>%
    group_by(Cluster, Genes) %>%
    summarise(MEAN = mean(Value)) %>%
    spread(value = MEAN, key = Cluster)
...
GSEA_Heatmap_Cluster <-
  function(X, pval = 0.05, es = 0, nes = -20, color = cm.colors(100), title = "", rmna = T, nPath=5, metrics = "NES", plotly = F, margin=c(0, 0, 0, 0), cexCol=1, cexRow=1, row_color=rainbow) {
    DF <- lapply(c("padj", "ES", "NES"), function(val) {
      ...
      df2 <-
        df %>%
        gather_(key = "Cells", value = val, colnames(df)[-1])
    })
    ...
    if (metrics == "NES") {
      ...
      MAT <-
        DF %>%
        dplyr::select(Pathway, Cells, NES) %>%
        spread(Cells, NES)
    }
    MATPadj <-
      DF %>%
      dplyr::select(Pathway, Cells, padj) %>%
      spread(Cells, padj)
    InMat <-
      MAT %>%
      dplyr::select(-Pathway) %>%
      as.matrix.data.frame() %>%
      set_rownames(MAT$Pathway)
    if (rmna) {
...
GSEA_Heatmap_SC <-
  function(X,
    ...
    if (metrics == "NES") {
      MAT <- DF %>% dplyr::select(Pathway, Cells, NES) %>% spread(Cells, NES)
    }
    else {
    ...
    newOrder <- DFORDER %>%
      ...
      map(function(x) {
        ...
        clus_gsea_matrix <-
          ...
          as.matrix() %>%
          apply(2, as.numeric) %>%
          set_rownames(clus_gsea_matrix$Pathway)
    ...
    LabROW1 <-
      ((SepROW[-(SepROW %>% length())] + Tab %>% cumsum()) / 2) %>%
      round(0) %>%
      magrittr::set_names(value = Tab %>% names())
    LabROW2 <- rep(NA, X$ExpressionMatrix %>% ncol())
    LabROW2[LabROW1] <- LabROW1 %>% names()
    row_side_color <-
      rep(Tab %>% names(), times = Tab) %>%
      factor() %>%
      data.frame()
    ...
    if (plotly) {
      heatmaply::heatmaply(
        ...
        na.rm = FALSE,
        heatmap_layers = c(
          lapply(max(SepROW) - SepROW, function(i)
            geom_hline(yintercept = i + 0.5, color = "black")),
...
Jaccard <- function(GeneSet) {
  sapply(X = GeneSet, FUN = function(Hall) {
    GeneSet %>% sapply(FUN = function(Hall2, Hall) {
      1 - ((intersect(Hall, Hall2) %>% length()) / (union(Hall, Hall2) %>% length()))
    }, Hall = Hall)
  }) %>% as.dist()
}
...
GSEA_remove_na <- function(x, rmna=T) {
  if (rmna) {
  ...
  function(X,
    ...
    if (plotly) {
      heatmaply(
        ...
        heatmap_layers = c(
          lapply(max(Bord) - Bord, function(i)
