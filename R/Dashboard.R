...
create_dashboard2 <- function(X)
{
  ...
  server <- function(input, output, clientData, session)
  {
    ...
    output$GeneSpace_Clus <- renderPlotly({
      ...
      Centroids <-
        X$cluster$coord_centroids %>% select_("Cluster", input$Axis1_Gene_Clus,
    ...
    output$Heatmap_Expression_Clus <- renderPlotly(X %>% Heatmap_Cluster(n = input$Num_Top_Genes, plotly = T))
  }
  return(shinyApp(ui, server))
}
...
create_dashboard3 <- function(X)
{
  ...
  server <- function(input, output, clientData, session)
  {
    ...
    output$Enrich_Heatmap <- (renderPlotly(
      ...
      {colo <- case_when(
        input$GSEA_Heatmap_Color  == "Heat" ~ heat.colors,
        input$GSEA_Heatmap_Color  == "CM" ~ cm.colors,
        input$GSEA_Heatmap_Color  == "RG" ~ gplots::redgreen,
        input$GSEA_Heatmap_Color  == "Spectral" ~ heatmaply::Spectral)


      GSEA_Heatmap_Cluster(X,
            ...
            navbarMenu(
              ...
              tabPanel(
                ...
                mainPanel(
                  width = 12,
                  align = "center",
                  plotlyOutput("Heatmap_Expression_Clus", width = "100%", height = "100%")
                )
              )
            )
          ))
