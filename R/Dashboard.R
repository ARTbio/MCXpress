...
create_dashboard1 <- function(X) {
  ...
  ui <- dashboardPage(
    ...
    dashboardBody(CSS,tabItems(tabItem(
      tabName = "mca",
      navbarPage(
        "MCA",
        navbarMenu(
  ...
  server <- function(input, output, clientData, session) {
    options(warn = -1)
    DR_axis_name <-
      X$MCA$cells_principal %>% select(contains("Axis")) %>%
      colnames
    ...
    output$CellSpaceGeneCor <- renderPlotly({
      if (input$DR_CS_AC_Type == "Principal") {
        axis_cor <-
          ...
          inner_join(
            X$ExpressionMatrix[input$DR_CS_AC_Gene,] %>% data.frame() %>% tibble::rownames_to_column() %>%
              set_colnames(c("Sample", "Expression")),
            by = "Sample"
          )
        p <-
          plot_ly(data = axis_cor,
                  x = ~ axis_cor[[input$DR_CS_AC_Axis_x]],
                  y = ~ axis_cor[[input$DR_CS_AC_Axis_y]]) %>%
          add_markers(
            text = ~ Sample,
            hoverinfo = "text",
            alpha = input$DR_CS_AC_Alpha,
            color = ~ Expression,
            marker = list(size = input$DR_CS_AC_Size)
          ) %>%
          layout(
            xaxis = list(title = input$DR_CS_AC_Axis_x),
        ...
        p <-
          ...
          layout(
            xaxis = list(title = input$DR_CS_Axis_x),
    ...
    output$CellSpace2D <- renderPlotly({
      if (input$DR_CS_2D_Type == "Principal") {
        ...
        p <- plot_ly(
          data = d3,
          x = ~ d3[[input$DR_CS_Axis_x]],
          y = ~ d3[[input$DR_CS_Axis_y]],
          type = "scatter",
          mode = "markers",
          text = ~ Sample,
          hoverinfo = "text",
    ...
    output$CellSpace3D <- renderPlotly(if (input$DR_CS_3D_Type ==
      ...
      plot_ly(
        ...
        text = ~ paste(
          ...
          ": ",
          X$MCA$cells_standard[[input$DR_CS_Axis1_3D]] %>%
            signif(digits = 4),
          "</br>",
          input$Axis2_3D,
          ": ",
      ...
      plot_ly(
        ...
        text = ~ paste(
          ...
          input$DR_CS_Axis1_3D,
          ": ",
          X$MCA$cells_principal[[input$DR_CS_Axis1_3D]] %>%
            signif(digits = 4),
          "</br>",
    ...
    output$TableGeneCor <-
      DT::renderDataTable(X$MCA$Axis_Gene_Cor %>%
                            extract(1:6))
...
create_dashboard2 <- function(X) {
  ...
  server <- function(input, output, clientData, session) {
    ...
    output$DTBOX <- DT::renderDataTable({
      ...
      Data <-
        X$cluster$gene_cluster_distances %>% gather("Cluster",
                                                    "Distance",-Genes)
      for (i in (Data$Cluster %>% unique)) {
        ...
        bin2 <-
          ...
          head(5) %>% paste(collapse = " ")
        DTboxplot <-
          bind_rows(DTboxplot, data_frame(Cluster,
                                          bin1, bin2))
      }
...
create_dashboard3 <- function(X) {
  ...
  server <- function(input, output, clientData, session) {
    ...
    output$CellSpace3D <-
      renderPlotly(if (input$DR_CS_3D_Type ==
                       "Standard") {
        plot_ly(
          X$MCA$cells_standard,
          mode = "markers",
    ...
    Table_Enrich2 <- eventReactive(
      input$Choice_Func_DT,
      Table_Enrich1()[[input$Choice_Func_DT]] %>% select(-nMoreExtreme,-leadingEdge),
      ignoreNULL = TRUE
    )
    ...
    output$Enrich_Heatmap <- (
      renderPlotly(
        Enrich_Boxplot %>%
          ...
          group_by(pathway) %>% plot_ly(
            x = ~ pathway,
            y = ~ Cluster,
            z = ~ (ES %>% as.numeric)
          ) %>% add_heatmap(zmin = -1,
                            zmax = 1) %>% layout(margin = list(
                              b = 100,
                              t = 25,
                              l = 150,
                              r = 150,
