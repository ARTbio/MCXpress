...
create_dashboard1 <- function(X) {
    ...
    ui <- dashboardPage(dashboardHeader(title = h1("MCXpress")),
        dashboardSidebar(sidebarMenu(menuItem("MCA", tabName = "mca",
            icon = icon("arrows")))), dashboardBody(tabItems(tabItem(tabName = "mca",
    ...
    server <- function(input, output, clientData, session) {
        ...
        output$CellSpace3D_Clus <- renderPlotly(plot_ly(X$MCA$cells_principal %>%
            ...
            hoverinfo = "text", marker = list(opacity = input$Alpha_3D_Clus,
                size = input$Size_3D_Clus, line = list(color = "rgba(0, 0, 0, .8)",
                  width = 2))) %>% add_markers() %>% layout(autosize = FALSE,
            legend = list(x = 100, y = 0.5), scene = list(xaxis = list(title = input$Axis1_3D_Clus),
        ...
        output$DTBOX <- DT::renderDataTable({
            ...
            Data <- X$cluster$gene_cluster_distances %>% gather("Cluster",
                "Distance", -Genes)
            for (i in (Data$Cluster %>% unique)) {
                Cluster <- i
                bin1 <- Data %>% filter(Cluster == i) %>% arrange_("Distance") %>%
                  separate(Genes, into = c("Genes", "bin"), sep = "-bin") %>%
                  filter(bin == 1) %>% extract2("Genes") %>%
                  head(5) %>% paste(collapse = " ")
                bin2 <- Data %>% filter(Cluster == i) %>% arrange_("Distance") %>%
                  separate(Genes, into = c("Genes", "bin"), sep = "-bin") %>%
                  filter(bin == 2) %>% extract2("Genes") %>%
                  head(5) %>% paste(collapse = " ")
                DTboxplot <- bind_rows(DTboxplot, data_frame(Cluster,
                  bin1, bin2))
            }
        ...
        Boxplot <- X$ExpressionMatrix %>% data.frame %>% rownames_to_column(var = "Genes") %>%
            ...
            gather("Sample", "Expression", -Genes) %>% as_tibble %>%
            arrange(Sample) %>% inner_join(X$cluster$labels,
            by = "Sample") %>% as_tibble
        output$Boxplot <- (renderPlotly(Boxplot %>% filter(Genes %in%
            input$Genes_Boxplot) %>% plot_ly(x = ~Genes, y = ~Expression) %>%
            add_trace(type = "box", color = ~Cluster, jitter = 0.5,
                pointpos = 0, boxpoints = "all", boxmean = "sd") %>%
            layout(boxmode = "group", margin = list(b = 100,
...
create_dashboard3 <- function(X) {
    ...
    ui <- dashboardPage(dashboardHeader(title = h1("MCXpress")),
        dashboardSidebar(sidebarMenu(menuItem("MCA", tabName = "mca",
            icon = icon("arrows")), menuItem("Clustering", tabName = "clus",
            icon = icon("object-group")), menuItem("GSEA", tabName = "gsea",
            icon = icon("gears")))), dashboardBody(tabItems(tabItem(tabName = "mca",
            navbarPage("MCA", navbarMenu("Cell Space", tabPanel("2 Axis",
                wellPanel(fluidRow(column(6, selectInput("DR_CS_2D_Type",
                  label = "Type", choices = c("Principal", "Standard"),
                  selected = "Principal"), selectInput("DR_CS_Axis_x",
            ...
            navbarPage("Clustering", navbarMenu("Cell Space",
                ...
                tabPanel(title = "Boxplot", fluidPage(titlePanel("Genes Expression by Cluster"),
                  ...
                  mainPanel(width = 12, h3("Boxplot"), plotlyOutput("Boxplot",
                    width = "100%", height = "100%"), h3("Genes Specific to Cluster"),
                    DT::dataTableOutput("DTBOX"))))), tabItem(tabName = "gsea",
            navbarPage("GSEA", tabPanel(title = "Enrichmentplot",
                fluidPage(titlePanel("Enrichmentplot"), wellPanel(fluidRow(column(width = 6,
                  selectInput("Geneset", "Choose a Geneset:",
                    choices = (X$GSEA$Pathways), selectize = TRUE)),
                  column(6, selectInput("Choice_Func_Plot", "Choose a Cluster:",
                    choices = X$GSEA$GSEA_Results %>% names),
                    selectInput("Mode_Func_Plot", "Choose", choices = c("Cluster",
                      "Axis"))))), mainPanel(width = 12, plotlyOutput("GSEA",
                ...
                fluidPage(titlePanel("Enrichment Table"), wellPanel(fluidRow(column(width = 6,
                  selectInput("Mode_Func_DT", "Choose Data:",
                    choices = (c("Cluster", "Axis")), selectize = TRUE)),
                  column(width = 6, selectInput("Choice_Func_DT",
                    "Choose a Geneset:", choices = (X$GSEA$GSEA_Results %>%
                      names), selectize = TRUE))))), mainPanel(width = 12,
                  DT::dataTableOutput("Table", width = "100%",
                    height = "100%"))), tabPanel(title = "Boxplot Enrichment Cluster",
                fluidPage(titlePanel("Genes Expression by Cluster"),
                  fluidRow(column(width = 10, selectInput(width = "100%",
                    "Enrich_Heatmap_GeneSet", "Choose a Gene:",
                    choices = (X$GSEA$Pathways), selectize = TRUE,
                    multiple = TRUE, selected = (X$GSEA$Pathways)[1])))),
                mainPanel(width = 12, h3("Enrichment Score Heatmap"),
    ...
    server <- function(input, output, clientData, session) {
        ...
        observeEvent(input$Mode_Func_Plot, {
            updateSelectInput(session, "Choice_Func_Plot", label = paste("Choose",
                ...
                switch(input$Mode_Func_Plot, Cluster = X$GSEA$GSEA_Results %>%
                  names, Axis = X$GSEA$GSEA_Results_Axis %>%
                  names)
            })
        }, ignoreNULL = TRUE)
        ...
        observeEvent(input$Mode_Func_DT, {
            updateSelectInput(session, "Choice_Func_DT", label = paste("Choose",
                ...
                switch(input$Mode_Func_DT, Cluster = X$GSEA$GSEA_Results %>%
                  names, Axis = X$GSEA$GSEA_Results_Axis %>%
                  names)
            })
        })
        ...
        Table_Enrich2 <- eventReactive(input$Choice_Func_DT,
            Table_Enrich1()[[input$Choice_Func_DT]] %>% select(-nMoreExtreme,
                -leadingEdge), ignoreNULL = TRUE)
        output$Table <- DT::renderDataTable(Table_Enrich2() %>%
        ...
        output$Enrich_Heatmap <- (renderPlotly(Enrich_Boxplot %>%
            filter(pathway %in% input$Enrich_Heatmap_GeneSet) %>%
            group_by(pathway) %>% plot_ly(x = ~pathway, y = ~Cluster,
            z = ~(ES %>% as.numeric)) %>% add_heatmap(zmin = -1,
            zmax = 1) %>% layout(margin = list(b = 100, t = 25,
            l = 150, r = 150, pad = 0))))
