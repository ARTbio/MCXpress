...
discretisation_bsplines <- function(X) {
    if (X %>% class %>% equals("MCXpress")) {
        ...
        bin2 <- TEST[1:nc, ] %>% set_colnames(paste0(TEST[1:nc, ] %>% colnames, "-bin2")) %>%
            set_rownames(exp_matrix %>% colnames())
        bin1 <- TEST[(nc + 1):(2 * nc), ] %>% set_colnames(paste0(TEST[1:nc, ] %>%
            colnames, "-bin1")) %>% set_rownames(exp_matrix %>% colnames())
        disjunctive_matrix <- cbind(bin1, bin2)
        disjunctive_matrix <- disjunctive_matrix[, disjunctive_matrix %>% colnames %>%
            sort]
...
discretisation_01 <- function(X, scaled = TRUE) {
    ...
    disjunctive_matrix <- disjunctive_matrix[disjunctive_matrix %>% rownames %>%
        sort, ] %>% t
    X$disjunctive_matrix <- disjunctive_matrix
    return(X)
}